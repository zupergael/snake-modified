...
.data          
    ...
    fruitseaten db 0        ; Counter for fruits eaten
    obstacles db 20 DUP(0)  ; Array to store obstacle positions (x,y pairs - 10 obstacles max)
...
main proc far
    ...
    ; --- MODIFIED: Reset game state for potential restart ---
    mov segmentcount, 1
    mov fruitactive, 1
    mov gameover, 0
    mov quit, 0
    ...
    mov byte ptr body+3, 0
    mov fruitseaten, 0      ; Reset fruits eaten counter
    mov obstaclecount, 0    ; Reset obstacle count
...
nevermind:
    ...
    cmp bl, 'v'
    je regenerate
    cmp bl, '#'          ; --- NEW: Check for obstacle collision
    je regenerate
...
; --- NEW PROCEDURE: Generate a new obstacle ---
generateobstacle proc
    ...
    ; Check if we can add more obstacles (max 10)
    cmp obstaclecount, 10
    jge skip_obstacle_gen
...
generate_new_obstacle:
    ...
    ; Check if position is valid (not on snake, fruit, or other obstacle)
    push dx
    call readcharat
    pop dx
    
    cmp bl, SOLID_BLOCK
    je generate_new_obstacle
    cmp bl, '^'
    je generate_new_obstacle
    cmp bl, '<'
    je generate_new_obstacle
    ...
    xor bh, bh
    mov bl, obstaclecount
    shl bx, 1  ; Multiply by 2 (each obstacle takes 2 bytes: x and y)
    lea si, obstacles
    add si, bx
    mov [si], dl      ; Store x position
    mov [si+1], dh    ; Store y position
...
out_draw:
    ...
    test cl, cl
    jz skip_obstacle_draw
    
    lea si, obstacles
draw_obstacles:
    mov dl, [si]      ; Get x position
    mov dh, [si+1]    ; Get y position
    mov bl, '#'       ; Obstacle character
    push cx
    push si
    call writecharat
    pop si
...
i_ate_fruit:    
    ...
    pop dx
    add bx, ax
    mov byte ptr ds:[bx], SOLID_BLOCK
    mov [bx+1], dx
    inc segmentcount 
    mov dh, fruity
    mov dl, fruitx
...
skip_speed_change:
    ...
    ; --- NEW: Check if we should spawn an obstacle every 2 fruits ---
    inc fruitseaten
    mov al, fruitseaten
    test al, 01h      ; Check if fruitseaten is even (bit 0 = 0)
    jnz skip_obstacle_spawn
